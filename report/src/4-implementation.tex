...
In this chapter we present all the informations to understein and to go into detail of the activity diagram in Figure~\ref{fig:implementation:activity_diagram} and the sequence diagram in Figure~\ref{fig:implementation:sequence_diagram}.
...
\begin{itemize}
    \item $R_{distance}=-\alpha|d(t)-d_{target}|$, where $d_{target}=10\ m$, where $\alpha = 1$
    \item $R_{comfort}=-\beta |a_E(t-1)-a_E(t)|$. We want penalize sudden break, where $\beta = 0.9$.
...
\end{itemize}
We immidiatly return a reward of $R_{terminal}=-1000$ if $d(t)\le0$ or if $d(t)>d_{max}=50\, m$. In both cases, the episode is immediately terminated.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Results}
In this Section we append some result report: 
