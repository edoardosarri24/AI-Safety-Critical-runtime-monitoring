...
\begin{itemize}
    ...
    \item \textbf{Complex Function} \\
        Is the module that cannot be certificated, usually an AI module. This AI component is composed by a data pre-processing, inference and post-processing; furthermore this componet can be implement with some degree of redundancy, with a voter at the end~\cite{abella2025safexplain}.
    ...
    \item \textbf{Safety Monitor} \\
        The assured function that is responsable to detect the fault behavior and send the command to the Switch. This module can observes both the RTA system and the Complex Function; this choice depend on the use case. This Monitor can perform the analysis on the AI module also in a hierarchical way (see Figure~\ref{fig:monitor:hierarchical_checker}): the low level monitor checks for evident error (e.g., out of distribution value) and the high level monitor checks for more semantics error~\cite{abella2025safexplain}.
    \item \textbf{Switch} \\
